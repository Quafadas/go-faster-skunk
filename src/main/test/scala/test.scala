...
class MySuite extends munit.FunSuite {
  ...
  test("all known types") {
    ...
    assertEquals(s, "int2 *: int4 *: int8 *: text *: numeric *: float4 *: float8 *: bool *: date *: time *: timetz *: timestamp *: interval *: bytea *: uuid *: int4.opt *: text.opt *: numeric.opt *: date.opt *: time.opt *: timetz.opt *: timestamp.opt *: interval.opt *: bytea.opt *: uuid.opt *: int2.opt *: float4.opt *: float8.opt *: bool.opt")
  }
